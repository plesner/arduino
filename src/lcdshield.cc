...
/*--------------------------------------------------------------------------------------
  Includes
--------------------------------------------------------------------------------------*/
#include "lib/LiquidCrystal.h"   // include LCD library
/*--------------------------------------------------------------------------------------
  Defines
--------------------------------------------------------------------------------------*/
// Pins in use
...
#define LCD_BACKLIGHT_OFF()     DigitalPin(LCD_BACKLIGHT_PIN).set_high(false)
#define LCD_BACKLIGHT_ON()      DigitalPin(LCD_BACKLIGHT_PIN).set_high(true)
#define LCD_BACKLIGHT(state)    { if( state ){DigitalPin(LCD_BACKLIGHT_PIN).set_high(true);}else{DigitalPin(LCD_BACKLIGHT_PIN).set_high(false);} }
/*--------------------------------------------------------------------------------------
  Variables
--------------------------------------------------------------------------------------*/
...
{
   //button adc input
   DigitalPin(BUTTON_ADC_PIN).set_mode(INPUT);         //ensure A0 is an input
   DigitalPin(BUTTON_ADC_PIN).set_high(false);      //ensure pullup is off on A0
   //lcd backlight control
   DigitalPin(LCD_BACKLIGHT_PIN).set_high(true);  //backlight control pin D3 is high (on)
   DigitalPin(LCD_BACKLIGHT_PIN).set_mode(OUTPUT);     //D3 is an output
   //set up the LCD number of columns and rows:
   lcd.begin( 16, 2 );
   //Print some initial text to the LCD.
   lcd.setCursor( 0, 0 );   //top left
   //          1234567890123456
   lcd.print( "Frxxtronixs 16x2" );
   //
...
byte ReadButtons()
{
   ...
   buttonVoltage = AnalogPin(BUTTON_ADC_PIN).read();
...
void loop()
{
   ...
   switch( button )
   {
     ...
     case BUTTON_SELECT:
     {
        ...
        DigitalPin(LCD_BACKLIGHT_PIN).set_high(false);
        Time::sleep_millis( 150 );
        DigitalPin(LCD_BACKLIGHT_PIN).set_high(true);   //leave the backlight on at exit
        Time::sleep_millis( 150 );
